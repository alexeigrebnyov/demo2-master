let message=''
function getAMGByDate() {
    let amgDays=[2,5]
    let ohDays=[1,6]
    let currDay= new Date(Date.now()).getDay()
    let amg = amgDays.includes(currDay )
    let oh = ohDays.includes(currDay )
    ...
    let urlist = amg ? ['/amgqc']:oh?['/hydroqc']:[]
    let namelist = amg? ['АМГ ']:oh?['17OH ']:[]
