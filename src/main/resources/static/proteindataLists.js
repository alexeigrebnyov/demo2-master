...
function getChartByDates( fOrg=0 ) {
    ...
    total_dates = []
    getServerData(fOrg)
    setTimeout(()=>{buildChart(current, 'tester')}, 1000)

}
function buildChart(dataArr=[], div_name='', slice=0) {
    ...
    dataArr.forEach(function (el){
        lot=el.lot
        plot_labels.push(lot)
        let l_dates=[]
        let l_vals=[]
        el.measureList.forEach(function (m){
            let md=new Date(m.measure_date)
            // console.log(min)
            // console.log(m.measure_date)
            // console.log(md)
            if (md>=new Date(from)&&md<=new Date(to)) {
                l_dates.push(md)
                l_vals.push(m.measure_val)
                total_val.push(m.measure_val)
                total_dates.push(md)
            }
    ...
    // total_val= total_val
   let sliced=total_val.slice(slice)
    ...
    layout = {
        title: 'Контроль белка Контроль №1 серия: '+lot,
        legend: {
            y: 1.0,
            x: 0.0,
            traceorder: 'normal',
...
function getServerData(fOrg=0) {
    let from = document.getElementById('dateFrom').value
    let to = document.getElementById('dateTo').value
    let urines = []
    if (current.length>0) {
        current.forEach(function (a) {
            a.measureList.forEach(function (b) {
                urines.push(new Date(b.measure_date))
            })
        })
        // .map(b=>b.measure_date)
        min = new Date(Math.min.apply(null, urines))
        max = new Date(Math.max.apply(null, urines))
    }



    if (from !== null && to !== null &&(new Date(from)<min||new Date(to)>max||current.length<1)) {
        fetch('/urine/getk1between/' + from + '/' + to + '/' + fOrg)
            .then((res) => res.json()
                .then((data) => {
                current=data
