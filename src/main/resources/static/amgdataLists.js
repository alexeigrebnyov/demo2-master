...
function getDataLists(dateFrom='', skip=true) {
    ...
    let set = new Set()
    let testSet = new Set()
    fetch("/amgqc/"+dateFrom).then((res) => res.json())
        .then((data) => {
...
function getChartByTestandLot(lot='', test='', date='', div_name='', slice=0){
    ...
    lot1=lot==='/'?'/test':lot
    fetch('/amgqc/getmeasuremap'+lot1+test+date)
        .then((res)=> res.json()
            .then((data)=>{
                if (div_name==='tester1') {
                    currData = data
...
function clearLists () {
    ...
    getChartByDateFrom('getmeasurelistsByDateFrom/2024-01-154T00:00', true,'/'+document.getElementById("lotseries").value,
        '/'+document.getElementById("testseries").value, '','tester1',0)
}
function getFirst() {

    getChartByDateFrom('getmeasurelistsByDateFrom/2024-01-15T00:00', true,'/'+document.getElementById("lotseries").value,
        '/'+document.getElementById("testseries").value, '','tester1', 0)
}

function addData() {
    fetch("/amgqc/saveLoadedMeasures").then()
    getInUse()
    setTimeout(
    getFirst, 1000)
}
