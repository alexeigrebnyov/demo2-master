...
//Получения списка данных и отрисовка графиков
function getChartByValue(end='', name='', from='', to='',parametr=''){
    ...
    fetch(parametr,
        {
            ...
            method: 'POST',
            body: JSON.stringify([end, from, to])
        })
        .then((res)=> res.json()
            .then((data)=>{
                total_val=[]
                let analizator_name=new Set()
                total_dates=[]
                cntrl_dates=[]
                let c=1
                data.forEach( function (some) {
                    analizator_name.add(some.analizator)
                    total_val.push(some.valueParam).toFixed(3)
                    total_dates.push(c++)
                })
...
function getGlukozaExpressValue(end='', name='', from='', to='',parametr=''){
    ...
    fetch(parametr,
        ...
        .then((res)=> res.json()
            .then((data)=>{
                ...
                data.forEach( function (some) {
                    let d = some.dateAnaliz
                    filial.add(some.filial)
                    total_val.push(some.valueAnaliz).toFixed(3)
                    total_dates.push(d)
...
// let divTest = '<button class="btn-danger" onclick="getChartByValue(\' select * from @table\',\n' +
//     '                         \'Все анализаторы\', document.getElementById(\'from\').value.toString(), document.getElementById(\'to\').value.toString())";>Все анализаторы</button>'


// function insertHTML(d='') {
//     document.getElementById('divForTest').innerHTML=d
// }
//
// insertHTML()