...
function initLotModalValues(end='', second='', third='') {
    fetch("/qc/"+end).then((res) => res.json())
        .then((data) => {
...
function updateLotModalValues(end='') {
    fetch("/qc/"+document.getElementById("updateInput").value+document.getElementById("lot").value).then((res) => res.json())
        .then((data) => {
...
function saveLotTester() {
    fetch("/qc/"+document.getElementById("saverInput").value,
        {
            headers: {
