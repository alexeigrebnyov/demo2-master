...
@RestController
@RequestMapping("/hivagqc")
public class HIVAgQCController {
  ...
  private HIVAgMeasureService measureService;
    ...
    public HIVAgLot getByInUse() {
        HIVAgLot lot = lotService.getByUse("1");
        lot.setDateFrom(LocalDate.now().atStartOfDay());
        return lot;
    }
    @GetMapping("/getTopLot/{lot}")
    public HIVAgLot getTopLot(@PathVariable("lot") String lot) {
        HIVAgLot lot1= new HIVAgLot(null, "ВЛК HIVAb",null, null, null, 0,null, "1");
        lot1.setDateFrom(LocalDate.now().atStartOfDay());
        HIVAgLot lot2 = lotService.getTopByLot(lot);
        if (lot2!=null) {
    ...
    public HIVAgTest getTestByInUse() {
        HIVAgTest test = testService.findByInuse("1");
        test.setDateFrom(LocalDate.now().atStartOfDay());
        return test;
    }

    @GetMapping("/getTopTest/{test}")
    public HIVAgTest getTopTest(@PathVariable("test") String test) {
        HIVAgTest test1= new HIVAgTest(null, "МБС HIV-AT/AG",null, null, null, 0,null, "1");
        test1.setDateFrom(LocalDate.now().atStartOfDay());
        HIVAgTest test2 = testService.getTopByLotOrderByNumberDesc(test);
        if (test2!=null) {
    ...
    @PostMapping("/savelot")
    public void saveLots(@RequestBody() HIVAgLot lot) {
        if (lot.getInuse().equals("1")) {
            for (HIVAgLot l: lotService.getLotsInuse("1")) {
                l.setInuse("0");
                lotService.save(l);
            }
        }
//        System.out.println(lot);
    ...
    public void saveTest(@RequestBody() HIVAgTest test) {
        if (test.getInuse().equals("1")) {
            for (HIVAgTest t : testService.getAllByInuse("1")) {
                t.setInuse("0");
                testService.save(t);
            }
//        System.out.println(lot);
    ...
    public void saveTest(HIVAgMeasure measure) {
//        System.out.println(lot);
...
//        System.out.println(measure);
    ...
    public String[] getInUse() {
        HIVAgLot l=lotService.getByUse("1");
        HIVAgTest t = testService.findByInuse("1");
    ...
    public List<HIVAgMeasure> getLotMeasure(@PathVariable("name") String name) {
        ...
        measureService.findAll()
                ...
                .forEach(measure -> {
                    ...
                    if (testMap.containsKey(testLot)) {
                        ...
                        List<HIVAgMeasure> measures = new ArrayList<>();
    ...
    public Map<Integer,List<HIVAgMeasure>> getMeasuresByAll(@PathVariable(name = "lot") String lot, @PathVariable(name = "test") String test,
    @PathVariable(name = "date") String date) {
        String[] dates =date.split(" ");
        Map<Integer, List<HIVAgMeasure>> testMap = new HashMap<>();
        measureService.findAll()
                .stream()
    ...
    public Map<String,HIVAgMeasure> getControlsByAll(@RequestBody CriteriaData data) {
        String[] dates =data.getDate().split(" ");
        Map<String, HIVAgMeasure> testMap = new HashMap<>();
        measureService.findAll()
                .stream()
    ...
    public Map<Integer,List<HIVAgMeasure>> getMeasures(@PathVariable(name = "lot") String lot, @PathVariable(name = "test") String test) {
        Map<Integer, List<HIVAgMeasure>> testMap = new HashMap<>();
              measureService.findAll()
                .stream()
                .filter(m-> m.getLot().getLot().equals(lot)&&m.getTest().getLot().equals(test))
...
//                          lotMap.put(measure.getLot().getNumber(), measure.getLot().getMeasureList());
    ...
    public Map<String,List<HIVAgMeasure>> getMeasuresByLot(@PathVariable(name = "lot") String lot) {
        Map<String, List<HIVAgMeasure>> testMap = new HashMap<>();
        measureService.findAll()
                .stream()
    ...
    public Map<Integer,List<HIVAgMeasure>> getMeasuresByTest(@PathVariable(name = "test") String test) {
        Map<Integer, List<HIVAgMeasure>> testMap = new HashMap<>();
        measureService.findAll()
                .stream()
    ...
    public List<HIVAgMeasureDTO> findByMeasure_dateAfter(@PathVariable(name = "date") String dateAfter) {
        ...
        return measureService.findAll()
                ...
                .map(MappingUtils::mapToHIVAgMeasureDto)
                .collect(Collectors.toList());
    }
    @GetMapping("/getmeasurelistsByLot/{lot}")
    public List<HIVAgMeasureDTO> findMeasureByLotlot(@PathVariable(name = "lot") String lot) {
    ...
    public List<HIVAgMeasureDTO> findMeasureByTestlot(@PathVariable(name = "test") String test) {
    ...
    public List<HIVAgMeasure> measureByTestLot(String testLot) {
...
//                }
    ...
    public void writeTestData()  {
        ...
        for (String s:series) {
            ...
            int count = 1;
            while (count < 11) {
                HIVAgTest t = new HIVAgTest(null,"МБС HIV-AT/AG",s, LocalDateTime.parse("2022-07-16T00:00"),
    ...
    public void writeLotData()  {
        ...
        for (String s:series) {
            ...
            int count = 1;
            while (count < 21) {
                HIVAgLot l = new HIVAgLot(null,"ВЛК HIVAg",s, LocalDateTime.parse("2022-07-16T00:00"),
                        LocalDateTime.parse("2023-07-16T00:00"), count, null, "0");
                lotService.save(l);
    ...
    public  void getScanData(@PathVariable("from") int from, @PathVariable("to") int to, @PathVariable("lot")String lot, @PathVariable("test") String test,
        ...
        File f = new File("c:/Users/Grebnev_A/Downloads/HIVAg.txt");
        ...
        while (sc.hasNextLine()&& count<484) {
            ...
            if (count > from&& count<to) {
                ...
                HIVAgLot l = lotService.findByNameAndLotAndNumber("ВЛК HIVAg", lot, lotBox);
                HIVAgTest t = testService.findByNameAndLotAndNumber("МБС HIV-AT/AG", test, box);
               HIVAgMeasure m = new HIVAgMeasure(null, "HIVAb", null,null,l,t);
                try {
